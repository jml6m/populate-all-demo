...
- **v1.0.3** (patch) — process/infra: agent-instruction consolidation (`AGENTS.md` single source, CODEOWNERS), Node ≥22 + MIT LICENSE, Dependabot multi-eco gold, PR-triggered `test.yml`, remove auto-link-issue workflow, and release push via **jml6m-version-bot** (`VERSION_BUMP_APP_*`). Reference `v1/` assets refreshed in place; core experiment fingerprint unchanged.
