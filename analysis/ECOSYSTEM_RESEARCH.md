...
The two acyclic **Yes** results (MikroORM, Hibernate) are the least-settled findings in this report and are treated as **provisional**.

**What the pass actually measures.** `ACYCLIC_PASS` means the schema-default fetch returned a graph that is _fully materialized and traversal-safe_ — a consumer can walk `A -> B -> C` without triggering any further database round-trips. It is **not** a claim that the graph was retrieved in a single SQL query. Neither passing library uses one query: both issue their multi-level loads _during the fetch_ — MikroORM fires a root select plus one batched `IN` select per relation level, and Hibernate's `@ManyToMany(fetch = EAGER)` fires eager sub-selects for the deeper levels. The `queryGate` samples its statement-count baseline _after_ the fetch, so it deliberately measures only traversal-time queries (the "no surprise N+1 in the view layer" property) and does not count fetch-time queries. A contrary argument — that Hibernate cannot hydrate a multi-level acyclic tree because a single static `@ManyToMany` join reaches only depth 1 and forces subsequent per-level SELECTs — is correct on the mechanics and fully consistent with this classification: the deeper levels _are_ loaded by additional queries; they simply fire at fetch time rather than on traversal.

The sandbox probes establish these results at small scale, but a stronger standard is warranted before treating "schema-driven acyclic full hydration is supported" as settled:
...
Accordingly, the acyclic **Yes** for MikroORM and Hibernate should be read as "fully materialized and traversal-safe at small scale in these probe versions," with a rigorous at-scale confirmation — external (wire/DB-level) query instrumentation that also counts fetch-time queries, a runtime/memory stress test, and a docs cross-check — left to follow-up work tracked in [issue #88](https://github.com/jml6m/populate-all-demo/issues/88) and [issue #90](https://github.com/jml6m/populate-all-demo/issues/90). The cyclic result and the seven acyclic **No** results do not depend on this and stand on their own.
