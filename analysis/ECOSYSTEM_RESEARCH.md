...
- **Documented semantics need confirmation.** Whether `populate: ['*']` and `@ManyToMany(fetch = EAGER)` are *documented* to fully resolve arbitrary-depth self-referential relations — versus happening to in these versions — should be verified against each library's official docs.
