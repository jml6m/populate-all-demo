...
The algorithmic foundation for cycle-safe full hydration is not a research frontier — it is a textbook two-pass pattern (see §1 and §4) with O(V+E) complexity and no corner cases. This research demonstrates that both Tarjan SCC Layering and Two-Pass Wire produce correct, fully wired graphs at every scale from 10 to 250,000 nodes. The problem is solved.

What is striking is that the enterprise ORM ecosystem has, almost without exception, elected not to make this the default. Every library surveyed above offloads the work to the developer: explicit depth caps in SQLAlchemy (`join_depth`), manual include chains in Hibernate (`join fetch`) and EF Core (`.Include()`), hardcoded relation paths in MikroORM, and silent N+1 fallback in ActiveRecord. The identity map — the very data structure that makes cycle-safe in-memory wiring trivially achievable — is present in all four backend ORMs, yet none uses it to provide automatic full hydration out of the box.

The standard defense is developer ergonomics: lazy loading keeps incidental queries from becoming silent performance disasters, and explicit includes surface SQL cost as a first-class code decision. These are real concerns. But the framing reveals the actual policy: _we have an identity map that can already wire cycles correctly in memory, and we are choosing not to activate it unless the developer enumerates every path by hand._ That is an ergonomic default, not an architectural constraint.

The gap between what is algorithmically possible and what ships as an ORM default is a deliberate product decision — and across virtually every library in this survey, that decision has consistently landed on the conservative side. Cycle-safe full hydration remains solved at the theory layer and absent at the defaults layer. For a class of tools whose central promise is to abstract away the impedance mismatch between relational storage and object graphs, leaving unbounded cyclic graph population as an unsupported opt-in is a significant omission.
