...
The **Acyclic (`A -> B -> C`)** column is verified directly by the sandbox probes. The **Cyclic (`A -> B -> A`)** column is documentation-driven — the probes verify only the acyclic case, and every surveyed library either requires explicit depth/path controls or rejects recursive eager self-relations outright for the cyclic case, so thee conclusions are derived from the documentation alone. See [Ecosystem Research §](./ECOSYSTEM_RESEARCH.md#2--backend-persistence-frameworks) for the full discussion.
...
| **SQLAlchemy**<br>_(Python)_      | v2.0.50 (SQLAlchemy)<br>v3.14.5 (Python)      | Cyclic (`A -> B -> A`): **No** — self-referential recursion is bounded by explicit loader-depth controls [[1]](https://docs.sqlalchemy.org/en/20/orm/self_referential.html). Acyclic (`A -> B -> C`): **No** — resolves topology but incurs lazy-load traversal queries (`queryGate=FAIL`).                                                                                                                                                           | **No schema-driven pass** |
