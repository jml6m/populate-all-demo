...
The four libraries with built-in identity maps (SQLAlchemy, Hibernate, EF Core, MikroORM) show mixed hydration results in v1. Hibernate is the exception: the Persistence Context correctly wires cyclic objects in memory, and the in-memory hydration check passes. SQLAlchemy and EF Core each have a partial hydration gap — their identity maps correctly wire loaded objects, but both require an explicit depth cap at the query layer to prevent unbounded recursive SQL joins. MikroORM's `populate: ['*']` wildcard halts at depth 1 without explicit paths, a query-API constraint rather than a fundamental identity-map limitation. Where hydration does succeed, a second failure appears at the serialization boundary: native serializers (`json.dumps`, Jackson, `System.Text.Json`) follow object references depth-first — when they hit a cycle, they either raise a circular-reference error, exhaust the stack, or trip a maximum-depth limit. Identity is preserved in memory but lost at serialization.

Applications still need an explicit cycle-aware serialization step (this repo's `cycle-flat` flat-form output, or a library reference handler such as EF Core's `ReferenceHandler.Preserve`) before a cyclic graph can leave memory. See [Ecosystem Research §2](./ECOSYSTEM_RESEARCH.md#2--backend-persistence-frameworks) for why most ORMs ship without this by default.
...
Both Tarjan SCC Layering and Two-Pass Wire successfully hydrate every tier from 10 to 250,000 nodes. At `stress` (50K), Tarjan is faster but uses more RAM; at `extreme` (250K), Two-Pass Wire is faster and still uses less RAM. The memory gap grows with scale because Tarjan SCC allocates extra bookkeeping structures for SCC membership and condensation edges.

The headline result is that cycle-safe graph population is **a solved problem at the algorithmic level**. Both Tarjan SCC Layering and Two-Pass Wire produce correct, fully wired graphs at every scale tested; either is suitable for production use. Why this hasn't translated into automatic full hydration as an ORM default across the ecosystem is a separate question — see [Ecosystem Research §2](./ECOSYSTEM_RESEARCH.md#2--backend-persistence-frameworks) for the design tradeoffs (lazy loading by default, explicit `.include()` ergonomics, and serialization-boundary concerns) that drive most ORMs toward explicit-include over automatic full hydration.
