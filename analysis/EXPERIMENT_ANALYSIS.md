...
But the downstream consumer problem remains unsolved by default. Across those same four ORMs,
native serializers (`json.dumps`, Jackson, `System.Text.Json`, `JSON.stringify`) fail on the
hydrated cyclic graph (`SERIALIZE_FAIL_CYCLE` or equivalent depth/cycle exceptions). EF Core's
own serializer guidance is explicit that `ReferenceHandler.Preserve` or `IgnoreCycles` must be
configured by the caller.

The practical pattern is universal: identity-map hydration is necessary, but not sufficient.
Applications still need an explicit cycle-aware serialization step (for example this repo's
Two-Pass Wire flat-form export, or an ORM-specific reference handler) before cyclic graphs can be
safely emitted as tree-shaped output.
...
Both Tarjan SCC Layering and Two-Pass Wire successfully hydrate every tier from 10 to 250,000
nodes. At `stress` (50K), Tarjan is faster but uses more RAM; at `extreme` (250K), Two-Pass Wire
is faster and still uses less RAM. The memory gap grows with scale because Tarjan SCC allocates
extra bookkeeping structures for SCC membership and condensation edges.
