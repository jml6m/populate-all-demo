...
The four libraries above with built-in identity maps (SQLAlchemy, Hibernate, EF Core, MikroORM) all pass the in-memory hydration check in v1: a single live object instance per logical row, even when traversed through cycles. None of them, however, can hand that hydrated graph to a default serializer. Native serializers (`json.dumps`, Jackson, `System.Text.Json`, `JSON.stringify`) follow object references depth-first — when they hit a cycle, they either raise a circular-reference error, exhaust the stack, or trip a maximum-depth limit. Identity is preserved in memory but lost at the serialization boundary.
