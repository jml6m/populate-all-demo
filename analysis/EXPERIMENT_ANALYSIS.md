...
| :-------------------------------- | :-------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Mongoose**<br>_(Node.js)_       | v8.3.0                                  | Relies on explicit `.populate()` chaining. No built-in `populateAll()` [[1]](https://mongoosejs.com/docs/populate.html)                                                                                                                                        | **Hydration Gap:** Developers must explicitly chain together `.populate()` parameters.                                                                                                                                                                                                          | [#16074](https://github.com/Automattic/mongoose/issues/16074) — Community feature request for a native schema-driven `populateAll()` that resolves all relations without a hardcoded depth cap. |
| **Sequelize**<br>_(Node.js)_      | v6.37.2                                 | `{ include: { all: true, nested: true } }` [[1]](https://sequelize.org/docs/v6/advanced-association-concepts/eager-loading/#including-everything) [[2]](https://sequelize.org/docs/v6/other-topics/constraints-and-circularities/)                             | **Hydration Gap:** In self-referential trees, this method arbitrarily truncates recursive objects at depth 1 to prevent infinite SQL joins.                                                                                                                                                     | [[2]](https://sequelize.org/docs/v6/other-topics/constraints-and-circularities/) — The official documentation acknowledges self-referential circular foreign keys as a known depth-truncation constraint; no upstream issue tracks lifting this limit. |
| **TypeORM**<br>_(Node.js)_        | v0.3.20                                 | `eager: true` via entity decorators [[1]](https://typeorm.io/eager-and-lazy-relations)                                                                                                                                                                         | **Hydration Gap:** Circular eager relations are disallowed. `eager: true` cannot be enabled on both sides of a bidirectional relation; a positive control with only one eager side succeeds.                                                                                                    | [#3663](https://github.com/typeorm/typeorm/issues/3663) — Long-standing open issue asking whether TypeORM should handle bidirectional recursive `eager: true` safely (e.g. by cycle-limiting) rather than crashing at initialization. |
| **Prisma**<br>_(Node.js)_         | v5.12.0                                 | Explicit `include` mapping [[1]](https://www.prisma.io/docs/orm/prisma-client/queries/relation-queries)                                                                                                                                                       | **Hydration Gap:** Each nesting level must be explicitly typed in the query argument. Data truncates at the specified depth.                                                                                                                                                                    | [#3725](https://github.com/prisma/prisma/issues/3725) — Open discussion on supporting recursive self-relations with dynamic depth in Prisma Client. The type system currently makes unbounded recursion inexpressible without raw SQL. |
| **MikroORM**<br>_(Node.js)_       | v6.6.13                                 | `populate: ['*']` [[1]](https://mikro-orm.io/docs/populating-relations#populating-all-relations)                                                                                                                                                               | **Hydration Gap:** `populate: ['*']` strictly halts at depth 1. Requires hardcoded paths for deeper traversal. MikroORM's Identity Map can correctly hydrate cyclic graphs when explicit paths are provided; the wildcard limitation is a query-API constraint, not a fundamental hydration failure. | [[1]](https://mikro-orm.io/docs/populating-relations#populating-all-relations) — The wildcard population depth cap is documented as intentional. Community discussion centers on whether deeper wildcard traversal can be made safe given MikroORM's Identity Map already avoids object duplication. |
| **SQLAlchemy**<br>_(Python)_      | v2.0.49                                 | `joinedload` with `join_depth` limits, or Bulk Loading via Identity Map [[1]](https://docs.sqlalchemy.org/en/20/orm/session_basics.html) [[2]](https://docs.sqlalchemy.org/en/20/orm/self_referential.html)                                                    | **Hydration Gap (Partial) & Serialization Gap:** No unbounded wildcard deep-fetch (recursive schemas require a `join_depth` limit to prevent infinite SQL joins). Identity Map bulk loads cleanly wire cyclic objects in memory. However, downstream serialization (`json.dumps`) still errors. | [[1]](https://docs.sqlalchemy.org/en/20/orm/self_referential.html) [[2]](https://docs.sqlalchemy.org/en/21/orm/relationship_persistence.html) — Official documentation covers both the `join_depth` constraint on self-referential queries and the `post_update=True` pattern for circular FK writes. Downstream serialization of cyclic objects is treated as an application-layer concern outside SQLAlchemy's scope. |
| **Hibernate/Jackson**<br>_(Java)_ | v6.6.0 (Hibernate)<br>v2.17.0 (Jackson) | Persistence Context acts as an Identity Map [[1]](https://javadoc.io/doc/com.fasterxml.jackson.core/jackson-databind/2.17.0/com/fasterxml/jackson/databind/ObjectMapper.html) [[2]](https://docs.hibernate.org/orm/6.6/introduction/html_single/#associations) | **Serialization Gap:** Persistence Context (L1 Cache) acts as an identity map and resolves memory cycles gracefully (e.g. A↔B). However, standard Jackson serialization throws `JsonMappingException` (Infinite Recursion). Workarounds mutate payloads or drop data.                           | [[1]](https://www.baeldung.com/jackson-bidirectional-relationships-and-infinite-recursion) — Widely referenced guide covering Jackson's annotation-based mitigations for cyclic serialization. Each option resolves the crash differently, with `@JsonIdentityInfo` being the only approach that preserves full graph fidelity. |
| **EF Core**<br>_(.NET)_           | v10.0.7                                 | Statically chained `.Include()` / `.ThenInclude()` [[1]](https://learn.microsoft.com/en-us/ef/core/querying/related-data/eager) [[2]](https://learn.microsoft.com/en-us/ef/core/querying/related-data/serialization)                                           | **Hydration Gap (Partial) & Serialization Gap:** Requires statically chained `.ThenInclude()` mapping (no unbounded wildcard). `ChangeTracker` safely wires cycles in RAM, but `System.Text.Json` throws `JsonException` upon hitting those cycles. Workarounds mutate payloads.                | [[2]](https://learn.microsoft.com/en-us/ef/core/querying/related-data/serialization) — Microsoft's official documentation addresses the serialization gap directly and documents `ReferenceHandler.IgnoreCycles` and `ReferenceHandler.Preserve` as built-in options, including explicit notes on each option's payload fidelity tradeoffs. |
| **ActiveRecord**<br>_(Ruby)_      | v8.1.3                                  | Explicit `.includes()` depth limits [[1]](https://api.rubyonrails.org/classes/ActiveRecord/Associations/ClassMethods.html) [[2]](https://docs.ruby-lang.org/en/3.3/JSON.html)                                                                                  | **Hydration & Serialization Gaps:** Hydration silently falls back to lazy-loading (N+1 queries) if traversal exceeds predefined `.includes()` depth. Built-in `.to_json` triggers `JSON::NestingError` or `SystemStackError`.                                                                   | [[2]](https://docs.ruby-lang.org/en/3.3/JSON.html) — Ruby's JSON documentation covers nesting limits. Community convention for cyclic schemas is to explicitly shape API responses via `as_json` options or custom serializer objects, rather than relying on recursive object serialization. |
...
The result is verified by two independent comparers. Both must pass for Stage 1 to succeed — if either fails, the run is reported as a hydration failure (or a conflict if one passes and the other fails):
...
- **`flatCompare`** — independently verifies correctness by building an identity map (object → index), converting the produced graph to an index-based flat representation, and comparing it entry-by-entry against the stored answer file. This catches bugs that could fool `smartCompare` by also affecting how the expected answer is loaded.

A result is reported as _double-verified_ only when both comparers **pass independently**. Neither comparer performs native `JSON.stringify` — they are correctness checks on the in-memory graph structure, not downstream consumer tests.
