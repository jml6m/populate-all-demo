...
The limitations of current hydration strategies are not unique to a single programming language or library but represent a pervasive challenge in the data-layer ecosystem. For this comparison, we now scope the question to **schema-driven full population** only:

- **graph_1 (cyclic):** `A -> B -> A`
- **graph_2 (acyclic):** `A -> B -> C`
...
| Library / Ecosystem               | Tested Version ([v1](../supporting-probes/results/reference/v1/)) | graph_1 (schema-driven cyclic) | graph_2 (schema-driven acyclic) | Classification |
| :-------------------------------- | :---------------------------------------------------------------- | :--------------------------------- | :---------------------------------- | :------------------------------------------------ |
| **Mongoose**<br>_(Node.js)_       | v8.24.0 | **No** — no schema-level recursive wildcard; explicit `.populate(...)` paths are required [[1]](https://mongoosejs.com/docs/populate.html) | **No** — `graph_2` probe fails smartCheck without explicit path chaining ([mongoose.json](../supporting-probes/results/reference/v1/mongoose.json)) | **Neither schema-driven cyclic nor schema-driven acyclic full hydration** |
| **Sequelize**<br>_(Node.js)_      | v6.37.8 | **No** — self-referential recursive expansion is constrained and requires explicit include structure [[1]](https://sequelize.org/docs/v6/advanced-association-concepts/eager-loading/#including-everything) [[2]](https://sequelize.org/docs/v6/other-topics/constraints-and-circularities/) | **No** — `graph_2` probe fails smartCheck without explicit include path declarations ([sequelize.json](../supporting-probes/results/reference/v1/sequelize.json)) | **Neither schema-driven cyclic nor schema-driven acyclic full hydration** |
| **TypeORM**<br>_(Node.js)_        | v0.3.30 | **No** — circular eager self-relations are unsupported/disallowed ([#3663](https://github.com/typeorm/typeorm/issues/3663)) | **No** — schema-driven eager self-relation setup overflows (`RangeError`) in `graph_2` probe ([typeorm.json](../supporting-probes/results/reference/v1/typeorm.json)) | **Neither schema-driven cyclic nor schema-driven acyclic full hydration** |
| **Prisma**<br>_(Node.js)_         | v5.22.0 | **No** — recursive self-relations require explicit nested `include` depth [[1]](https://www.prisma.io/docs/orm/prisma-client/queries/relation-queries) ([#3725](https://github.com/prisma/prisma/issues/3725)) | **No** — `graph_2` probe fails smartCheck without explicit nested include paths ([prisma.json](../supporting-probes/results/reference/v1/prisma.json)) | **Neither schema-driven cyclic nor schema-driven acyclic full hydration** |
| **MikroORM**<br>_(Node.js)_       | v6.6.14 | **No** — cyclic traversal still requires explicit depth handling; schema-only cyclic full hydration is not provided [[1]](https://mikro-orm.io/docs/populating-relations#populating-all-relations) | **Yes** — `graph_2` probe passes hydration + serialization with schema-driven wildcard configuration ([mikroorm.json](../supporting-probes/results/reference/v1/mikroorm.json)) | **Hydration Pass (acyclic only) & Serialization Pass (acyclic only)** |
| **SQLAlchemy**<br>_(Python)_      | v2.0.51 | **No** — self-referential recursion is bounded by explicit loader-depth controls [[1]](https://docs.sqlalchemy.org/en/20/orm/self_referential.html) | **No** — `graph_2` probe resolves topology but incurs lazy-load traversal queries (`queryGate=FAIL`) ([sqlalchemy.json](../supporting-probes/results/reference/v1/sqlalchemy.json)) | **Neither schema-driven cyclic nor schema-driven acyclic full hydration** |
| **Hibernate/Jackson**<br>_(Java)_ | v6.6.0.Final (Hibernate)<br>v2.17.0 (Jackson) | **No** — cyclic full hydration requires explicit per-level fetch shape, not unbounded schema-only traversal [[1]](https://docs.hibernate.org/orm/6.6/introduction/html_single/#associations) | **Yes** — `graph_2` probe passes hydration + serialization using schema-driven eager association defaults ([hibernate.json](../supporting-probes/results/reference/v1/hibernate.json)) | **Hydration Pass (acyclic only) & Serialization Pass (acyclic only)** |
| **EF Core**<br>_(.NET)_           | v8.0.6 | **No** — cyclic recursive include requires explicit shape; no schema-only unbounded traversal [[1]](https://learn.microsoft.com/en-us/ef/core/querying/related-data/eager) | **No** — schema-driven `AutoInclude` on self-reference is rejected by cycle guard (`InvalidOperationException`) in `graph_2` probe ([efcore.json](../supporting-probes/results/reference/v1/efcore.json)) | **Neither schema-driven cyclic nor schema-driven acyclic full hydration** |
| **ActiveRecord**<br>_(Ruby)_      | v8.1.3 | **No** — recursive eager loading requires explicit `.includes(...)` depth declarations [[1]](https://api.rubyonrails.org/classes/ActiveRecord/Associations/ClassMethods.html) | **No** — `graph_2` probe resolves topology but incurs lazy-load traversal queries (`queryGate=FAIL`) ([activerecord.json](../supporting-probes/results/reference/v1/activerecord.json)) | **Neither schema-driven cyclic nor schema-driven acyclic full hydration** |
