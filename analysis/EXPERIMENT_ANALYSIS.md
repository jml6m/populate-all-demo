...
The limitations of current hydration strategies are not unique to a single programming language or library but represent a pervasive challenge in the data-layer ecosystem. One example is found in the Mongoose JS library supporting MongoDB ([Issue #16074](https://github.com/Automattic/mongoose/issues/16074)), which, in the tested version v8.3.0, does not have built-in support for automatic, schema-driven population of all referenced paths in the model. Looking at the broader ORM/ODM landscape, related cyclic-reference and recursive-population challenges also appear across other data libraries — with different mechanisms and constraints visible in their issue trackers and documentation:

| Library                      | Docs & Eager Loading Strategy                                                                                                                                                                                                                                                                            | Behavior on Circular/Recursive Relations (Functional Gaps)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Tracked Issue / Discussion                                                                                                        |
| :--------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------- |
| **Mongoose**<br>_(Node.js)_  | [Population docs](https://mongoosejs.com/docs/populate.html)<br>No built-in `populateAll()`; relies on explicit `.populate()` chaining.<br>**Tested version:** v8.3.0                                                                                                                                    | **No built-in schema-driven full-depth population.** There is no automatic recursive `populateAll()`; developers must explicitly chain `.populate()` and manually manage depth/path expansion.                                                                                                                                                                                                                                                                                                                                                                                                                                    | [#16074](https://github.com/Automattic/mongoose/issues/16074) — Schema-driven `populateAll()`; cites circular refs as motivation. |
| **Sequelize**<br>_(Node.js)_ | [Including Everything](https://sequelize.org/docs/v6/advanced-association-concepts/eager-loading/#including-everything), [Constraints](https://sequelize.org/docs/v6/other-topics/constraints-and-circularities/)<br>Supports `{ include: { all: true, nested: true } }`.<br>**Tested version:** v6.37.2 | **Not automatic full-depth on self-recursive paths.** In the refined self-referential test, `{ include: { all: true, nested: true } }` populated the first child level but did not automatically continue through deeper levels without explicit path definition.                                                                                                                                                                                                                                                                                                                                                                 | —                                                                                                                                 |
| **TypeORM**<br>_(Node.js)_   | [Eager & Lazy Relations](https://typeorm.io/eager-and-lazy-relations)<br>Supports `eager: true` via entity decorators.<br>**Tested version:** v0.3.20                                                                                                                                                    | **Circular eager on both sides is explicitly unsupported.** `eager: true` cannot be enabled on both sides of a recursive bidirectional relation; a positive control with only one eager side succeeds.                                                                                                                                                                                                                                                                                                                                                                                                                            | [#3663](https://github.com/typeorm/typeorm/issues/3663) — `eager: true` on recursive relation causes infinite loop.               |
| **Prisma**<br>_(Node.js)_    | [Self-relations](https://www.prisma.io/docs/orm/prisma-schema/data-model/relations/self-relations)<br>No native "include all" or dynamic hydration.<br>**Tested version:** v5.12.0                                                                                                                       | **No automatic full-depth recursive include.** Each nesting level must be explicitly encoded in the query; arbitrary recursive depth is not schema-driven automatically.                                                                                                                                                                                                                                                                                                                                                                                                                                                          | [#3725](https://github.com/prisma/prisma/issues/3725) — Feature Request: Support recursive relationships in queries.              |
| **MikroORM**<br>_(Node.js)_  | [Populating relations](https://mikro-orm.io/docs/populating-relations)<br>Supports wildcards `populate: ['*']`.<br>**Tested version:** v6.6.13                                                                                                                                                           | **No automatic full-depth recursion.** Handles cyclic serialization gracefully without crashing, but `['*']` strictly stops at Depth 1. To hydrate arbitrary-depth recursive graphs, developers must explicitly define exact nesting paths (e.g., `populate: ['friends.friends.friends']`), exposing the same static-depth limitations as other Node ORMs.                                                                                                                                                                                                                                                                        | —                                                                                                                                 |
| **SQLAlchemy**<br>_(Python)_ | [Session Basics](https://docs.sqlalchemy.org/en/20/orm/session_basics.html), [Self-Referential & `join_depth`](https://docs.sqlalchemy.org/en/20/orm/self_referential.html)<br>Utilizes `joinedload` with `join_depth`, or Bulk Loading via Identity Map.<br>**Tested version:** v2.0.49                 | **Requires explicit depth bounds & Serialization crashes.** No unbounded wildcard deep-fetch (recursive schemas require a `join_depth` limit to prevent infinite SQL joins). While the Identity Map gracefully wires memory cycles natively, downstream serializers (like `json.dumps`) still crash on these graphs.                                                                                                                                                                                                                                                                                                              | —                                                                                                                                 |
| **Hibernate**<br>_(Java)_    | <a href="https://hibernate.org">Fetching strategies</a><br>Supports `FetchType.EAGER` and Entity Graphs.<br>**Tested version:** v6.6 (Current) / Jackson v2.17.0                                                                                                                                         | **Jackson infinite recursion.** Persistence Context (L1 Cache) acts as an identity map and resolves memory cycles gracefully (e.g. A↔B). However, downstream Spring Boot/Jackson serialization throws `JsonMappingException` (Infinite Recursion) when traversing these cycles. Workarounds (`@JsonIgnore`, `@JsonIdentityInfo`) either strictly truncate the data or mutate the JSON payload into pointer schemas. **Note:** Hibernate separately blocks "unbounded eager loading" on multiple collections (MultipleBagFetchException) to prevent Cartesian products, but the specific crash here is due to serialization depth. | —                                                                                                                                 |
| **EF Core**<br>_(.NET)_      | [Eager Loading](https://learn.microsoft.com/en-us/ef/core/querying/related-data/eager), [Related Data & Serialization](https://learn.microsoft.com/en-us/ef/core/querying/related-data/serialization)<br>Supports explicit `.Include()` / `.ThenInclude()`.<br>**Tested version:** v10.0.203             | **No dynamic wildcard & JSON cyclic exceptions.** Requires statically chained `.ThenInclude()` mapping (no unbounded wildcard). The `ChangeTracker` safely wires cycles in RAM, but `System.Text.Json` throws `JsonException` upon hitting those cycles. Official workarounds (`IgnoreCycles` or `[JsonIgnore]`) truncate the output data, while `Preserve` mutates the JSON format with `$ref` metadata, often breaking standard frontend clients.                                                                                                                                                                               | —                                                                                                                                 |
