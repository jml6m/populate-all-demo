...
| :-------------------------------- | :-------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------ |
| **Mongoose**<br>_(Node.js)_       | v8.24.0                                                                     | Cyclic (`A -> B -> A`): **No** — no schema-level recursive wildcard; explicit `.populate(...)` paths are required [[1]](https://mongoosejs.com/docs/populate.html).<br>Acyclic (`A -> B -> C`): **No** — probe fails smartCheck without explicit path chaining.                                                                                                                                                                                          | **No schema-driven pass** |
| **Sequelize**<br>_(Node.js)_      | v6.37.8                                                                     | Cyclic (`A -> B -> A`): **No** — self-referential recursive expansion is constrained and requires explicit include structure [[1]](https://sequelize.org/docs/v6/advanced-association-concepts/eager-loading/#including-everything) [[2]](https://sequelize.org/docs/v6/other-topics/constraints-and-circularities/).<br>Acyclic (`A -> B -> C`): **No** — fails smartCheck without explicit include paths.                                              | **No schema-driven pass** |
| **TypeORM**<br>_(Node.js)_        | v0.3.30                                                                     | Cyclic (`A -> B -> A`): **No** — circular eager self-relations are unsupported/disallowed ([#3663](https://github.com/typeorm/typeorm/issues/3663)).<br>Acyclic (`A -> B -> C`): **No** — the schema-level eager mechanism is not constructible for a self-referential relation: `eager: true` expands the self-join without bound and overflows at query construction (`RangeError`), independent of the data (it fails identically on an empty table). | **No schema-driven pass** |
| **Prisma**<br>_(Node.js)_         | v5.22.0                                                                     | Cyclic (`A -> B -> A`): **No** — recursive self-relations require explicit nested `include` depth [[1]](https://www.prisma.io/docs/orm/prisma-client/queries/relation-queries) ([#3725](https://github.com/prisma/prisma/issues/3725)).<br>Acyclic (`A -> B -> C`): **No** — fails smartCheck without explicit nested include paths.                                                                                                                     | **No schema-driven pass** |
| **MikroORM**<br>_(Node.js)_       | v6.6.14                                                                     | Cyclic (`A -> B -> A`): **No** — schema-only cyclic full hydration is not provided [[1]](https://mikro-orm.io/docs/populating-relations#populating-all-relations).<br>Acyclic (`A -> B -> C`): **Yes** — passes hydration + serialization with schema-driven wildcard configuration.                                                                                                                                                                     | **Acyclic-only pass**     |
| **SQLAlchemy**<br>_(Python)_      | <small>v2.0.50 (SQLAlchemy)</small><br><small>v3.14.5 (Python)</small>      | Cyclic (`A -> B -> A`): **No** — self-referential recursion is bounded by explicit loader-depth controls [[1]](https://docs.sqlalchemy.org/en/20/orm/self_referential.html).<br>Acyclic (`A -> B -> C`): **No** — resolves topology but incurs lazy-load traversal queries (`queryGate=FAIL`).                                                                                                                                                           | **No schema-driven pass** |
| **Hibernate/Jackson**<br>_(Java)_ | <small>v6.6.0.Final (Hibernate)</small><br><small>v2.17.0 (Jackson)</small> | Cyclic (`A -> B -> A`): **No** — cyclic full hydration requires explicit per-level fetch shape [[1]](https://docs.hibernate.org/orm/6.6/introduction/html_single/#associations).<br>Acyclic (`A -> B -> C`): **Yes** — passes hydration + serialization using schema-driven eager association defaults.                                                                                                                                                  | **Acyclic-only pass**     |
...
| **ActiveRecord**<br>_(Ruby)_      | v8.1.3                                                                      | Cyclic (`A -> B -> A`): **No** — recursive eager loading requires explicit `.includes(...)` depth declarations [[1]](https://api.rubyonrails.org/classes/ActiveRecord/Associations/ClassMethods.html).<br>Acyclic (`A -> B -> C`): **No** — resolves topology but incurs lazy-load traversal queries (`queryGate=FAIL`).                                                                                                                                 | **No schema-driven pass** |
