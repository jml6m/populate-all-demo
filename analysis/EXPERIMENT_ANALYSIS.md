...
| **TypeORM**<br>_(Node.js)_        | v0.3.30                                       | Cyclic (`A -> B -> A`): **No** — circular eager self-relations are unsupported/disallowed ([#3663](https://github.com/typeorm/typeorm/issues/3663)). Acyclic (`A -> B -> C`): **No** — schema-driven eager self-relation setup overflows (`RangeError`) in probe execution.                                                                                   | **No schema-driven pass** |
...
| **SQLAlchemy**<br>_(Python)_      | v2.0.51                                       | Cyclic (`A -> B -> A`): **No** — self-referential recursion is bounded by explicit loader-depth controls [[1]](https://docs.sqlalchemy.org/en/20/orm/self_referential.html). Acyclic (`A -> B -> C`): **No** — resolves topology but incurs lazy-load traversal queries (`queryGate=FAIL`).                                                                  | **No schema-driven pass** |
...
| **EF Core**<br>_(.NET)_           | v8.0.6                                        | Cyclic (`A -> B -> A`): **No** — cyclic recursive include requires explicit shape [[1]](https://learn.microsoft.com/en-us/ef/core/querying/related-data/eager). Acyclic (`A -> B -> C`): **No** — schema-driven `AutoInclude` on self-reference is rejected by cycle guard (`InvalidOperationException`).                                                  | **No schema-driven pass** |
