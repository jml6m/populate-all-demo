...
import { PopulateAlgorithm } from './algorithms/types';

const algorithms: PopulateAlgorithm[] = [
  naiveRecursion,
  customMapTracker
];
...
function runBenchmark() {
  ...
  for (const dataset of datasets) {
    ...
    const testData = loadYaml(`${dataset}_test.yaml`);
    ...
    for (const algo of algorithms) {
      ...
      try {
        ...
        accuracyResult = {
          pass: false,
          errorDetail: error.message || "Fatal Execution Error",
          nodesProcessed: 0
        };
      }
