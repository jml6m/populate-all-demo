...
describe('isAlreadyUpToDate — dataset/report completeness', () => {
  ...
  it('returns false when the latest run index is malformed even if an older run was valid', () => {
    const tempDir = makeTempDir();
    const reportRelPath = 'basic/benchmark-1.json';
...
describe('dataset-output-format-matrix — generate audit log', () => {
  ...
  it('writes logs/local/<run-id>/dataset-output-format-matrix.log with all display states', () => {
...
// ---------------------------------------------------------------------------
// Repo-structure smoke test
// ---------------------------------------------------------------------------

describe('repo-structure smoke — README layout', () => {
  ...
  it('no README.md in logs/reference/ (reference dirs use root README only)', () => {
    assert.ok(
      !fs.existsSync(path.join(ROOT, 'logs', 'reference', 'README.md')),
      'logs/reference/README.md must not exist — document reference rules in the root README instead'
    );
  });

  it('no README.md in reports/reference/ (reference dirs use root README only)', () => {
    assert.ok(
      !fs.existsSync(path.join(ROOT, 'reports', 'reference', 'README.md')),
      'reports/reference/README.md must not exist — document reference rules in the root README instead'
    );
  });

  it('no README.md in supporting-probes/results/reference/ (reference dirs use root README only)', () => {
    assert.ok(
      !fs.existsSync(path.join(ROOT, 'supporting-probes', 'results', 'reference', 'README.md')),
      'supporting-probes/results/reference/README.md must not exist — document reference rules in the root README instead'
    );
  });
});
